...
$(document).ready(function () {
    ...
    let DrawSparkline = function () {
        // Line Chart
        let lineColors = ['#00acc1', '#f1556c'];
        let lineDataColors = $("#lifetime-sales").data('colors');
        if (lineDataColors) {
            lineColors = lineDataColors.split(",");
        }
        $('#lifetime-sales').sparkline([0, 23, 43, 35, 44, 45, 56, 37, 40], {
            ...
            fillColor: hexToRGB(lineColors[0], 0.3),
        ...
        $('#lifetime-sales').sparkline([25, 23, 26, 24, 25, 32, 30, 24, 19], {
            ...
            fillColor: hexToRGB(lineColors[1], 0.3),
            composite: true,
            highlightLineColor: 'rgba(0,0,0,.1)',
            highlightSpotColor: 'rgba(0,0,0,.2)',
            maxSpotColor: false,
        ...
        // Bar Chart
        let barColors = ['#00acc1'];
        let barDataColors = $("#income-amounts").data('colors');
        if (barDataColors) {
        ...
        // Pie Chart
        let pieColors = ['#00acc1', '#4b88e4', '#e3eaef', '#fd7e14'];
        let pieDataColors = $("#total-users").data('colors');
        if (pieDataColors) {
            pieColors = pieDataColors.split(",");
        }
...
//various examples
let colors = ['#6658dd'];
let dataColors = $("#world-map-markers").data('colors');
if (dataColors) {
